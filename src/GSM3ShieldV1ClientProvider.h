...
class GSM3ShieldV1ClientProvider : public GSM3MobileClientProvider,  public GSM3ShieldV1BaseProvider
{
	private:
		...
		void ftpCloseSessionContinue();
		
		void ufsCloseFileContinue();		
	...
	public:
		...
		/** Configures FTP parameter and open the connection
			@param server: address of FTP server
			@param port: port of FTP server
			@param username: authentication user identification string for FTP
			@param password: authentication password for FTP
			@return 0 if command running, 1 if success, otherwise error
		 */		
		int ftpOpenSession(char* server, int port, char* username, char* password);
		...
		/** Open file saved in UFS (User File Storage)
			...
			@return 1 if ok, 0 otherwise
		 */		
		int ufsOpenFile(char* flashFile);
		
		/** Read from file Open in UFS
			@param length: length of bytes to read from file opened
			@param ufsFilehandle: Filehandle
			@return 1 if ok, 0 otherwise
		 */		
		void ufsReadFile(int length);
		
		int ufsAvailable();
		/** Close the file opened in UFS (User File Storage)
			@return 1 if ok, 0 otherwise
		 */	
		int ufsCloseFile();
		
		char ufsReadFileByte();
