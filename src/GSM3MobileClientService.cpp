...
int GSM3MobileClientService::connectFTP(char* host, int port, char* user, char* password)
{
	if(theGSM3MobileClientProvider==0)
	return 2;
	
	int res=theGSM3MobileClientProvider->ftpOpenSession(host, port, user, password);
	if(flags & GSM3MOBILECLIENTSERVICE_SYNCH)
	res=waitForAnswer();
...
int GSM3MobileClientService::stopFTP()
{
	if(theGSM3MobileClientProvider==0)
	return 2;
	
	int res=theGSM3MobileClientProvider->ftpCloseSession();
	if(flags & GSM3MOBILECLIENTSERVICE_SYNCH)
	res=waitForAnswer();
	return res;
}

int GSM3MobileClientService::openFile(char* fileUFS)
{
	if(theGSM3MobileClientProvider==0)
	return 2;
	
	int res=theGSM3MobileClientProvider->ufsOpenFile(fileUFS);
	if(flags & GSM3MOBILECLIENTSERVICE_SYNCH)
	res=waitForAnswer();
...
void GSM3MobileClientService::readFile(int blockSize)
{
	theGSM3MobileClientProvider->ufsReadFile(blockSize);
}

int GSM3MobileClientService::availableFile()
{
	int res=theGSM3MobileClientProvider->ufsAvailable();
	return res;
}

char GSM3MobileClientService::readByteFile()
{
	char c=theGSM3MobileClientProvider->ufsReadFileByte();
	return c;
}

int GSM3MobileClientService::closeUFS()
{
	if(theGSM3MobileClientProvider==0)
	return 2;
	
	int res=theGSM3MobileClientProvider->ufsCloseFile();
	if(flags & GSM3MOBILECLIENTSERVICE_SYNCH)
	res=waitForAnswer();
	return res;
}
long GSM3MobileClientService::fileSize()
{
	long res=theGSM3MobileClientProvider->ftpFilesize();
	return res;
}
