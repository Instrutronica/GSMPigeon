...
//Flush all SMS main function.
void GSM3ShieldV1SMSProvider::flushAllSMS()
{
	...
	theGSM3ShieldV1ModemCore.openCommand(this,FLUSHALLSMS);
	flushAllSMSContinue();
}
//FLush all SMS continue function.
void GSM3ShieldV1SMSProvider::flushAllSMSContinue()
{
	bool resp;
	// 1: Deleting SMS
	// 2: wait for OK
	switch (theGSM3ShieldV1ModemCore.getCommandCounter()) {
		case 1:
		theGSM3ShieldV1ModemCore.setCommandCounter(2);
		theGSM3ShieldV1ModemCore.genericCommand_rq(PSTR("AT+QMGDA=\"DEL ALL\""), true);
		break;
		case 2:
		if(theGSM3ShieldV1ModemCore.genericParse_rsp(resp))
		{
